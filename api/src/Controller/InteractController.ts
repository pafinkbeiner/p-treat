            ...
            jwt.verify(token, process.env.JWT_SECRET || "", async(err:any, authData: any) => {
                ...
                if(err){
                    ...
                    if(found){
                        ...
                        if(site){
                            await site.updateOne({$set: {rating: {like: site.rating.like + 1, disklike: site.rating.disklike - 1}}})
                        }else{
...
export const performDislike = (req: Request, res: Response) => {
    if(req.params.siteId != undefined){
                ...
                if(err){
                    ...
                    if(!found){
                        ...
                        if(user){
                            ...
                            await user.updateOne({$pull: { liked: site?._id }})
